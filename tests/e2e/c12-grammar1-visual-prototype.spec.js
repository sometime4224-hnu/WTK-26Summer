...
async function answerCurrentQuestionWrong(page) {
  ...
  const wrongChoice = await page.evaluate(() => {
    ...
    const item = window.C12_GRAMMAR1_VISUAL_PROTOTYPE.quiz.find((q) => q.q === prompt);
...
test.describe('c12 grammar1 visual version', () => {
  ...
  test('loads every storyboard image', async ({ page }) => {
    await expect(page.locator('.story-card img')).toHaveCount(5);
    ...
    expect(loaded.map((item) => item.src)).toEqual([
      '../assets/c12/grammar/images/grammar1-visual-prototype/core-concept.webp',
      '../assets/c12/grammar/images/grammar1-visual-prototype/form-change.webp',
      '../assets/c12/grammar/images/grammar1-visual-prototype/experience-result.webp',
      '../assets/c12/grammar/images/grammar1-visual-prototype/state-change.webp',
      '../assets/c12/grammar/images/grammar1-visual-prototype/aseo-comparison.webp'
    ]);
  ...
  test('opens storyboard images in a fullscreen viewer', async ({ page }) => {
    await expect(page.locator('.fullscreen-btn')).toHaveCount(5);
    ...
    await expect(page.locator('#lightboxTitle')).toContainText('내가 해 봤더니 결과가 생겼어요');
    await expect(page.locator('#lightboxImage')).toHaveAttribute('src', '../assets/c12/grammar/images/grammar1-visual-prototype/core-concept.webp');
  ...
  test('supports the six shared translation languages', async ({ page }) => {
    for (const lang of LANGUAGES) {
      ...
      await expect(page.locator(`.story-card .lang-box[data-lang="${lang.code}"].lang-visible`)).toHaveCount(5);
