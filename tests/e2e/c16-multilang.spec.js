...
test('C16 어휘 28개는 6개 번역과 정확한 lang/dir 메타데이터를 제공한다', async ({ page }) => {
  ...
  const coverage = await page.evaluate((languages) => {
    ...
    return {
      ...
      assets: words.map((word) => ({
        id: word.id,
        image: word.image,
        quizImage: word.quizImage
      })),
  ...
  expect(coverage.assets).toEqual(Array.from({ length: 28 }, (_, index) => {
    const id = `c16-vocab-${String(index + 1).padStart(2, '0')}`;
    const path = `../assets/c16/vocabulary/images/cards-v2/${id}.webp`;
    return { id, image: path, quizImage: path };
  }));
  expect(coverage.missing).toEqual([]);
  await expect(page.locator('.word-card')).toHaveCount(28);

  const firstCard = page.locator('.word-card').first();
  const firstCardImage = firstCard.locator('.word-card__image');
  await expect(firstCardImage).toHaveAttribute('src', /cards-v2\/c16-vocab-01\.webp$/);
  await expect.poll(() => firstCardImage.evaluate((image) => image.naturalWidth)).toBe(256);
  ...
  await page.locator('[data-quiz-mode="image"]').click();
  const quizImage = page.locator('.quiz-image');
  await expect(quizImage).toHaveAttribute('src', /cards-v2\/c16-vocab-\d{2}\.webp$/);
  await expect.poll(() => quizImage.evaluate((image) => image.naturalWidth)).toBe(256);
