...
- 앱 런타임에서 참조하는 `split/` 에셋 219개를 WebP near-lossless 95로 전환했다. `source/` PNG 원본은 보존했다.
...
- 정식 매니페스트의 런타임 이미지 219개가 모두 WebP로 디코딩됨.
...
| 대상 | 변환 전 | 변환 후 | 절감 |
| --- | ---: | ---: | ---: |
| 런타임 `split/` 에셋 219개 | 71.38MB PNG | 41.94MB WebP | 약 41% |

변환 방식: `cwebp -near_lossless 95 -q 100`

원본 제작/재분할용 PNG는 `source/`에 남겨 두고, 앱이 읽는 매니페스트 경로만 WebP로 바꾸었다.
