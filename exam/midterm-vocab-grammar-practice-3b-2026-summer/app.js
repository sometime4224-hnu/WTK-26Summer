...
function renderLandingIndex() {
    const roundContainer = document.getElementById("roundBoard");
    const reviewContainer = document.getElementById("reviewBoard");

    if (roundContainer) {
        roundContainer.innerHTML = renderLandingCards(ROUND_LINKS);
    }

    if (reviewContainer) {
        reviewContainer.innerHTML = renderLandingCards(REVIEW_LINKS);
    }
}

function renderLandingCards(items) {
    return items.map((item) => {
        const totalQuestions = getRoundQuestionTotal(item.id);
        const savedSelections = readStoredSelections(getRoundStorageKey(item.id));
        const answeredCount = Object.keys(savedSelections).length;
        const summary = loadRoundSummary(item.id);
        const attemptText = summary.attempts > 0 ? `응시 ${summary.attempts}회` : "응시 전";
        ...
        return `
            <a class="round-card${cardClass}" href="${item.href}">
                <span class="round-card__label">${item.label}</span>
                <span class="round-card__badge">${stateText}</span>
                <span class="round-card__status">${attemptText}</span>
                <span class="round-card__status">${bestText}</span>
