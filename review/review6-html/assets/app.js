(function () {
  var DATA = window.REVIEW6_DATA;
  var FEEDBACK_I18N = window.REVIEW6_FEEDBACK_I18N || {};
  var STORAGE_PREFIX = "snukorean:review6";
  var runtime = {
  ...
  function renderHub() {
    var cards = DATA.order.map(function (sectionId) {
      ...
      return (
        ...
        '<div>' +
        '<div class="eyebrow">REVIEW 6</div>' +
    ...
    document.getElementById("app").innerHTML =
      ...
      '<div class="eyebrow">복습 6</div>' +
      '<h1 class="hero-title">16~18과 복습 퀴즈</h1>' +
      '<p class="hero-copy">스마트폰 폭이 기본이고, 오른쪽 위 토글로 태블릿 폭으로 바로 전환됩니다. 확인하기, 평가하기, 읽기 파트가 자동 저장되며 결과와 과거 기록을 다시 열어볼 수 있습니다.</p>' +
      '<div class="stats-grid">' +
      '<div class="stats-card"><span class="stat-label">파트</span><span class="stat-value">3</span></div>' +
  ...
  function renderResult(section) {
    ...
    document.getElementById("app").innerHTML =
      ...
      "</div>" +
      (section.nextHref ? '<div class="section-actions" style="margin-top:12px;"><a class="text-button" href="' + escapeHtml(section.nextHref) + '">다음 파트</a></div>' : "") +
      "</section>" +
  ...
  function renderReviewItems(attempt, results) {
    return results.map(function (item) {
      ...
      return (
        ...
        "</div>" +
        '<p class="review-prompt"><strong>' + escapeHtml(item.number) + ".</strong> " + renderRichText(item.prompt) + "</p>" +
        contextHtml +
        transcriptHtml +
  ...
  function renderContext(context, compact) {
    ...
    return (
      ...
      context.map(function (block) {
        ...
        return '<div class="context-block' + type + '">' + renderRichText(block.text).replace(/\n/g, "<br>") + "</div>";
      }).join("") +
      "</div>"
    );
  }
  ...
  function renderRichText(value) {
    return escapeHtml(value)
      .replace(/\[\[u:([^\]]+)\]\]/g, '<span class="text-underline">$1</span>')
      .replace(/_{3,}/g, '<span class="blank-line">$&</span>')
      .replace(/\( {2,}\)/g, '<span class="blank-line blank-line--paren">(     )</span>');
  }
