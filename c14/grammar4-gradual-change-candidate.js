(() => {
    ...
    function renderCard(card) {
        ...
        return `
            <article class="motion-card" data-card-id="${escapeAttr(card.id)}" data-direction="${escapeAttr(card.direction)}" data-visual-state="${visualState}" data-stage="${stage}" style="--energy:${energy};">
                <div class="card-top">
                    <span class="mode-tag mode-tag--${escapeAttr(card.direction)}" data-copy-layer="PRIMARY">${escapeHtml(modeLabel)}</span>
                    <span class="stage-chip" data-copy-layer="PRIMARY">${stage}${escapeHtml(stageUnit)}</span>
                </div>
                ${scene}
                <div class="copy">
                ...
                <div class="card-actions">
                    <button class="step-btn" type="button" data-action="advance" data-card-id="${escapeAttr(card.id)}" data-copy-layer="PRIMARY">
    ...
    function renderImageScene(card, stage, showBurst) {
        ...
        return `
            <div class="media-frame" data-nontext-visual data-visual-state="${getVisualState(stage)}" data-copy-layer="PRIMARY">
                <img class="scene-base" src="${escapeAttr(card.baseImage)}" alt="${escapeAttr(card.baseAlt)}" decoding="async" fetchpriority="auto" />
                ...
                <div class="object-layer">
                    ${patchInstances.map((patch) => `
                        <div class="object-patch is-visible" style="left:${patch.x}%;top:${patch.y}%;width:${patch.w}%;--rotate:${patch.rotate}deg;--patch-scale:${patch.scale};">
                            <img src="${escapeAttr(patch.src)}" alt="" decoding="async" fetchpriority="auto" />
                        </div>
                    `).join("")}
                </div>
    ...
    function renderEffectScene(card, stage, energy, showBurst) {
        ...
        return `
            <div class="media-frame effect-frame effect-frame--${escapeAttr(card.visual.kind)}" data-nontext-visual data-visual-state="${getVisualState(stage)}" data-copy-layer="PRIMARY" style="--level:${level};--energy:${energy};">
                <span class="scene-chip" data-copy-layer="PRIMARY">${escapeHtml(card.sceneLabel)}</span>
                ${showBurst ? renderClickWordEffect(card) : ""}
    ...
    function renderAmountScene(card, stage, energy, showBurst) {
        ...
        return `
            <div class="media-frame amount-frame amount-frame--${escapeAttr(card.visual.kind)}" data-nontext-visual data-visual-state="${getVisualState(stage)}" data-copy-layer="PRIMARY" style="--level:${level};--amount:${amount};--energy:${energy};--shrink-scale:${shrinkScale};">
                <span class="scene-chip" data-copy-layer="PRIMARY">${escapeHtml(card.sceneLabel)}</span>
                ${showBurst ? renderClickWordEffect(card) : ""}
    ...
    function advanceCard(cardId, focusAction = null) {
        ...
        saveStages();
        render(focusAction ? { cardId, action: focusAction } : null);
    }
    ...
    function getVisualState(stage) {
        return stage > 0 ? "changing" : "initial";
    }
        ...
        try {
            ...
            CARDS.forEach((card) => {
                ...
                empty[card.id] = Number.isFinite(value)
                    ? Math.min(Math.max(Math.trunc(value), 0), MAX_STORED_STAGE)
                    : 0;
            });
        } catch (error) {
            return empty;
